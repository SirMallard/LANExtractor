...
class FileExplorer():
	...
	def render(self, window_flags: int) -> None:
		...
		table_flags: int = imgui.TableFlags_.sortable.value | imgui.TableFlags_.row_bg.value
		if imgui.begin_table("##file_table", 2, table_flags):
			imgui.table_setup_column("Name", imgui.TableColumnFlags_.width_stretch.value, 0.8)
			imgui.table_setup_column("Size", imgui.TableColumnFlags_.width_stretch.value, .2)
